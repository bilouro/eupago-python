...
def test_capture_success(client: EupagoClient) -> None:
    ...
    result = client.credit_card.capture(transaction_id="txn-auth-001")
...
def test_capture_failure(client: EupagoClient) -> None:
    ...
    result = client.credit_card.capture(transaction_id="txn-bad")
...
def test_charge_subscription_success(client: EupagoClient) -> None:
    respx.post(f"{SUBSCRIPTION_PAYMENT_URL}/12345").mock(
        return_value=Response(
            200,
            json={
                "transactionStatus": "Success",
...
async def test_capture_async_success(client: EupagoClient) -> None:
    ...
    result = await client.credit_card.capture_async(transaction_id="txn-async-auth")
    assert result.status == PaymentStatus.PAID
    await client.aclose()
